package dataStructure.graph.map;

import dataStructure.graph.CommonGraph;
...
public class MapGraph<V, E> extends CommonGraph<V, E> {
    ...
    public Collection<Edge<V, E>> incomingEdges(V vert) {
        ...
        for (MapVertex<V, E> mapVertex : mapVertices.values()) {

            Collection<Edge<V, E>> outVertsCollection = mapVertex.getAllOutEdges();
            for (Edge<V, E> edge : outVertsCollection) {
                V actualVert = edge.getVDest();
                if (actualVert.equals(vert)) {
