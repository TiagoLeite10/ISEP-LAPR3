...
-- Tabela hub
CREATE TABLE hub (
codHub INTEGER CONSTRAINT pkHubCodHub PRIMARY KEY,
hub VARCHAR2(5) 
    CONSTRAINT nnHubHub NOT NULL
    CONSTRAINT ukHubHub UNIQUE,
tipoHub CHAR(1) CONSTRAINT nnHubTipoHub NOT NULL,
CONSTRAINT ckHubHub CHECK (REGEXP_LIKE (hub, 'CT[0-9][0-9]?[0-9]?')),
CONSTRAINT ckHubTipoHub CHECK (tipoHub = 'E' OR tipoHub = 'P')
);
...
codHub INTEGER
    CONSTRAINT fkVendaCodHub REFERENCES hub (codHub)
    CONSTRAINT nnVendaCodHub NOT NULL,
