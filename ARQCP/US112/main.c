/* Ficheiro principal para a resolução da US 112 */
#include <stdio.h>
#include <stdlib.h>
	...
	if (create_sensors == 1) {
		...
		while (option_secondary_menu != 0) {
			...
			switch (option_secondary_menu) {
				...
				case 6:
					printf("Indique o nome do ficheiro para exportar os dados e leituras de cada um dos sensores: \n");
					char file_name_data[100];
					scanf("%s", file_name_data);

					header_csv(file_name_data);
					export_sensor_data(file_name_data, ptr_sensores_temp, num_sens_temp);
					export_sensor_data(file_name_data, ptr_sensores_velc_vento, num_sens_velc_vento);
					export_sensor_data(file_name_data, ptr_sensores_dir_vento, num_sens_dir_vento);
					export_sensor_data(file_name_data, ptr_sensores_humd_atm, num_sens_humd_atm);
					export_sensor_data(file_name_data, ptr_sensores_humd_solo, num_sens_humd_solo);
					export_sensor_data(file_name_data, ptr_sensores_pluvio, num_sens_pluvio);			

					printf("\nDados e leituras de cada um dos sensores exportados com sucesso!\n");
					
					break;

				case 7:
					printf("Indique o nome do ficheiro para exportar os dados da matriz diária: \n");
					char file_name[100];
					scanf("%s", file_name);
					...
					if (export_daily_matrix (file_name, daily_matrix, NUM_TYPE_SENSORS) == 0) {
						printf("\nNão foi possível exportar os dados da matriz diária!\n");
					} else {
						printf("\nDados da matriz diária exportados com sucesso!\n");
					}
